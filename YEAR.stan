...
for(k in 1:Nyear) {
  ...
  for(j in 1:n) {
     ...
     ndvi_true[j,1] ~ normal(B_int[ID[j]],sigma);//initial condition of state variable
        ...
        ndvi[j,1] ~ normal(ndvi_true[j,1]+ slope_mm*mm[j,1]+year_effect[1],sigma_mm); //initial condition of observed data
      
    for (i in 2:Nyear) {
   
     ndvi_true[j,i] ~ normal(ndvi_true[j,i-1]*(1+r1[ID[j]]*(1-ndvi_true[j,i-1]/K)),sigma); //process model
      
      ndvi[j,i]~normal(ndvi_true[j,i] + slope_mm*mm[j,i]+dum_years[i]*year_effect[i],sigma_mm); //measurement error model
    }
  }
 ...
 generated quantities{ //posterior predictions for model validation and visualization
  ...
  for(jj in 1:n){
    for(ii in 1:Nyear){
      ...
      ndvi_pred[jj,ii] = ndvi_true[jj,ii] + slope_mm*mm[jj,ii]+dum_years[ii]*year_effect[ii];
